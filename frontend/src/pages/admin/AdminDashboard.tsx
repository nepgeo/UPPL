...
useEffect(() => {
  getAdminDashboard(selectedSeasonId || undefined)
    .then((res) => {
            ...
            <main className="flex-1 min-w-0">
              ...
              <div className="lg:hidden mb-6">
                <Select value={activeTab} onValueChange={setActiveTab}>
                  ...
                  <SelectContent
                    className="
                      rounded-lg shadow-lg bg-blue-600/70 backdrop-blur-md max-h-64 overflow-y-auto
                      scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent border border-white/20
                    "
                  >
                    {[
                    { value: "overview", label: "Overview", icon: LayoutDashboard },
                      ...
                      return (
                        <SelectItem
                          ...
                          className={`
                            ...
                            whitespace-nowrap text-white/90 focus:bg-white focus:text-blue-600
                            ${activeTab === tab.value
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-4">
              {[
                ...
                >
                  <div className={`bg-gradient-to-br ${stat.gradient} p-2 sm:p-4 text-white text-center`}>
                    <stat.icon className="w-4 h-4 sm:w-6 sm:h-6 mb-1 sm:mb-2 opacity-80 group-hover:opacity-100 transition-opacity mx-auto" />
                    <p className="text-4xl sm:text-2xl font-bold">{stat.value}</p>
                    <p className="text-[9px] sm:text-xs font-medium opacity-80 mt-0.5 uppercase">{stat.title}</p>
                  </div>
                </Card>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <Card className="border-0 rounded-xl shadow-md overflow-hidden">
                <CardHeader className="bg-gradient-to-r from-blue-600 to-purple-600 text-white py-2 sm:py-3 px-4 sm:px-5">
                  <CardTitle className="text-xs sm:text-sm font-semibold flex items-center justify-center gap-2">
                    <Image className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> <span className="uppercase">Content Overview</span>
                  </CardTitle>
                </CardHeader>
                <CardContent className="p-3 sm:p-5 space-y-2 sm:space-y-3">
                  {[
                    ...
                    <div key={i} className="flex items-center justify-between px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg bg-gray-50">
                      <div className="flex items-center gap-2 sm:gap-3">
                        <item.icon className={`w-3.5 h-3.5 sm:w-5 sm:h-5 ${item.color}`} />
                        <div>
                          <p className="text-xs sm:text-base font-medium text-gray-700 uppercase">{item.label}</p>
                          {item.sub && <p className="text-[9px] sm:text-xs text-gray-400 uppercase">{item.sub}</p>}
                        </div>
                      </div>
                      <span className="text-base sm:text-2xl font-bold text-gray-800">{item.value}</span>
                    </div>
                  ))}
                </CardContent>
              </Card>
              ...
              {/* Player Roles Card */}
              <Card className="border-0 rounded-xl shadow-md overflow-hidden">
                <CardHeader className="bg-gradient-to-r from-green-500 to-emerald-600 text-white py-2 sm:py-3 px-4 sm:px-5">
                  <CardTitle className="text-xs sm:text-sm font-semibold flex items-center justify-center gap-2">
                    <BarChart3 className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> <span className="uppercase">Player Roles</span>
                  </CardTitle>
                </CardHeader>
                    ...
                    return (
                      <div>
                        <ResponsiveContainer width="100%" height={140}>
                          <BarChart data={rolesData} barSize={24}>
                            <XAxis dataKey="name" tick={{ fontSize: 9, textTransform: 'uppercase' }} axisLine={false} tickLine={false} />
                            <YAxis hide />
                            <Tooltip formatter={(val: number) => [`${val} (${total > 0 ? Math.round(val / total * 100) : 0}%)`, 'Count']} />
                            <Bar dataKey="value" radius={[6, 6, 0, 0]} />
                          </BarChart>
                        </ResponsiveContainer>
                        <div className="flex flex-wrap justify-center gap-2 sm:gap-3 mt-2 text-[10px] sm:text-sm text-gray-500 uppercase">
                          {rolesData.map(r => (
                            <span key={r.name} className="flex items-center gap-1">
                              <span className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full" style={{ backgroundColor: r.fill }} />
                              {r.value} {r.name}
                            </span>
                          ))}
                        </div>
              ...
              <Card className="border-0 rounded-xl shadow-md overflow-hidden">
                <CardHeader className="bg-gradient-to-r from-indigo-500 to-violet-600 text-white py-2 sm:py-3 px-4 sm:px-5">
                  <CardTitle className="text-xs sm:text-sm font-semibold flex items-center justify-center gap-2">
                    <Activity className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> <span className="uppercase">Match Status</span>
                  </CardTitle>
                </CardHeader>
                <CardContent className="p-3 sm:p-5">
                  {(() => {
                    ...
                    return (
                      <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-4">
                        <ResponsiveContainer width="100%" height={130}>
                          <PieChart>
                            <Pie data={matchData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={30} outerRadius={50} paddingAngle={3}>
                              {matchData.map((_, i) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card className="border-0 rounded-xl shadow-md overflow-hidden">
                <CardHeader className="bg-gradient-to-r from-sky-500 to-blue-600 text-white py-3 px-5 sm:flex sm:flex-row sm:items-center sm:justify-between">
                  <CardTitle className="text-sm font-semibold flex items-center justify-center sm:justify-start gap-2">
                    <UserPlus className="w-4 h-4" /> <span className="uppercase">Recent Registrations</span>
                  </CardTitle>
                  <button onClick={() => setActiveTab('users')} className="hidden sm:flex text-xs text-blue-200 hover:text-white items-center gap-1 transition-colors uppercase mt-2 sm:mt-0">
                ...
                <CardContent className="p-0">
                    ...
                    <div className="divide-y divide-gray-100">
                      {dashboardData.recentUsers.map((u: any, i: number) => (
                        <div key={u._id || i} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 transition-colors">
                          <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-gradient-to-br from-sky-400 to-blue-500 flex items-center justify-center text-white text-xs md:text-sm font-bold shrink-0 overflow-hidden">
                            {u.profileImage ? (
                              <img src={getProfileImageUrl(u.profileImage)} alt={u.name} className="w-full h-full object-cover"
                                onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                            ) : null}
                            <span>{u.name?.charAt(0)?.toUpperCase() || '?'}</span>
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-sm md:text-base font-medium text-gray-800 truncate">{u.name}</p>
                  ...
                  <button onClick={() => setActiveTab('users')} className="sm:hidden w-full py-3 text-center text-sm font-semibold text-blue-600 hover:bg-blue-50 border-t border-gray-100 transition-colors uppercase">
              ...
              <Card className="border-0 rounded-xl shadow-md overflow-hidden">
                <CardHeader className="bg-gradient-to-r from-orange-500 to-amber-600 text-white py-3 px-5 flex flex-row items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-xs text-orange-100 hover:text-white flex items-center gap-1 transition-colors uppercase"
                  >
          ...
          <TabsContent value="teams">
            <div className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                {/* Active Season Card — first */}
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  <div className="bg-gradient-to-r from-emerald-500 to-teal-500 px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-center gap-3">
                    <div className="w-8 h-8 sm:w-9 sm:h-9 lg:w-10 lg:h-10 rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center">
                      <Trophy className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 text-white" />
                    </div>
                    <div>
                      <h3 className="text-sm sm:text-base lg:text-lg font-bold text-white uppercase">Active Season</h3>
                      <p className="text-[10px] sm:text-xs lg:text-sm text-emerald-100 uppercase">Currently active season</p>
                    </div>
                  </div>
                  <div className="p-4 sm:p-5 lg:p-6">
                    {seasons.find((s: any) => s.isCurrent) ? (
                      <div className="text-center mb-4">
                        <div className="w-14 h-14 sm:w-16 sm:h-16 lg:w-20 lg:h-20 rounded-2xl bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center mx-auto mb-3 shadow-lg shadow-emerald-200">
                          <span className="text-xl sm:text-2xl lg:text-3xl font-bold text-white">{seasons.find((s: any) => s.isCurrent)?.number}</span>
                        </div>
                        <p className="text-sm sm:text-base lg:text-lg font-semibold text-gray-800">Season {seasons.find((s: any) => s.isCurrent)?.number}</p>
                        <p className="text-xs sm:text-sm text-gray-400 mt-0.5 uppercase">
                          Deadline: {new Date(seasons.find((s: any) => s.isCurrent)?.endDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
                        </p>
                      </div>
                    ) : (
                      <div className="text-center mb-4">
                        <div className="w-14 h-14 sm:w-16 sm:h-16 lg:w-20 lg:h-20 rounded-2xl bg-gray-100 flex items-center justify-center mx-auto mb-3">
                          <Trophy className="w-7 h-7 sm:w-8 sm:h-8 lg:w-10 lg:h-10 text-gray-300" />
                        </div>
                        <p className="text-sm sm:text-base lg:text-lg font-medium text-gray-400">No Active Season</p>
                      </div>
                    )}
                    <div className="border-t border-gray-100 pt-4">
                      <label className="text-xs sm:text-sm font-semibold text-gray-600 mb-1.5 block uppercase">Switch Active Season</label>
                      <div className="flex gap-2">
                        <Select value={seasonInput.number} onValueChange={(val) => {
                          ...
                          <SelectTrigger className="flex-1 h-9 sm:h-10 lg:h-11 rounded-xl text-xs sm:text-sm lg:text-base border-gray-200">
                        ...
                        <Button onClick={handleSetSeason} className="h-9 sm:h-10 lg:h-11 rounded-xl px-3 sm:px-4 lg:px-6 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-xs sm:text-sm lg:text-base font-semibold shadow-md shadow-emerald-200 uppercase">
                          Set
                        </Button>
                      </div>
                    </div>
                ...
                {/* Season Management Card — second, lg:col-span-2 */}
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden lg:col-span-2">
                  <div className="bg-gradient-to-r from-blue-600 via-blue-500 to-indigo-600 px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-center gap-3">
                    <div className="w-8 h-8 sm:w-9 sm:h-9 lg:w-10 lg:h-10 rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center">
                      <Plus className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 text-white" />
                    </div>
                    <div>
                      <h3 className="text-sm sm:text-base lg:text-lg font-bold text-white uppercase">Season Management</h3>
                      <p className="text-[10px] sm:text-xs lg:text-sm text-blue-100 uppercase">Create and manage league seasons</p>
                    </div>
                  </div>
                  <div className="p-4 sm:p-5 lg:p-6 flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-2 sm:gap-3">
                    <Button
                      onClick={() => { setNewSeason({ number: '', endDate: '' }); setShowCreateModal(true); }}
                      className="h-9 sm:h-10 lg:h-11 rounded-xl px-4 sm:px-5 lg:px-6 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-xs sm:text-sm lg:text-base font-semibold shadow-md shadow-blue-200 uppercase"
                    >
                    ...
                    <Button
                      ...
                      className="h-9 sm:h-10 lg:h-11 rounded-xl px-4 sm:px-5 lg:px-6 text-xs sm:text-sm lg:text-base font-semibold border-gray-200 hover:bg-gray-50 uppercase"
                    >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-3 sm:p-4 lg:p-5 flex flex-col items-center gap-1 sm:gap-2">
                  <div className="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 rounded-xl bg-blue-50 flex items-center justify-center">
                    <Calendar className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 text-blue-500" />
                  </div>
                  <div className="text-center">
                    <p className="text-base sm:text-lg lg:text-2xl font-bold text-gray-900">{seasons.length}</p>
                    <p className="text-[9px] sm:text-[10px] lg:text-xs font-medium text-gray-400 uppercase tracking-wider">Total Seasons</p>
                  </div>
                </div>
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-3 sm:p-4 lg:p-5 flex flex-col items-center gap-1 sm:gap-2">
                  <div className="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 rounded-xl bg-emerald-50 flex items-center justify-center">
                    <Trophy className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 text-emerald-500" />
                  </div>
                  <div className="text-center">
                    <p className="text-base sm:text-lg lg:text-2xl font-bold text-gray-900">{seasons.filter((s: any) => s.isCurrent).length}</p>
                    <p className="text-[9px] sm:text-[10px] lg:text-xs font-medium text-gray-400 uppercase tracking-wider">Active</p>
                  </div>
                </div>
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-3 sm:p-4 lg:p-5 flex flex-col items-center gap-1 sm:gap-2">
                  <div className="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 rounded-xl bg-purple-50 flex items-center justify-center">
                    <Clock className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 text-purple-500" />
                  </div>
                  <div className="text-center">
                    <p className="text-base sm:text-lg lg:text-2xl font-bold text-gray-900">{seasons.filter((s: any) => !s.isCurrent).length}</p>
                    <p className="text-[9px] sm:text-[10px] lg:text-xs font-medium text-gray-400 uppercase tracking-wider">Past</p>
                  </div>
                </div>
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-3 sm:p-4 lg:p-5 flex flex-col items-center gap-1 sm:gap-2">
                  <div className="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 rounded-xl bg-amber-50 flex items-center justify-center">
                    <AlertCircle className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 text-amber-500" />
                  </div>
                  <div className="text-center">
                    <p className="text-base sm:text-lg lg:text-2xl font-bold text-gray-900">{seasons.filter((s: any) => new Date(s.endDate) < new Date() && !s.isCurrent).length}</p>
                    <p className="text-[9px] sm:text-[10px] lg:text-xs font-medium text-gray-400 uppercase tracking-wider">Expired</p>
                  </div>
                </div>
              </div>
