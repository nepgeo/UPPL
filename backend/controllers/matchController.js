...
const updateMatchResult = async (req, res) => {
  try {
    ...
    // Sync the score field for consistency
    if (!match.score) match.score = { teamA: {}, teamB: {} };
    match.score.teamA = {
      battingTeam: 'teamA',
      runs: safeNumber(teamAResult.runs),
      wickets: safeNumber(teamAResult.wickets),
      balls: Math.floor(parseOvers(teamAResult.overs) * 6),
      extras: match.score?.teamA?.extras || 0,
      fours: match.score?.teamA?.fours || 0,
      sixes: match.score?.teamA?.sixes || 0,
      runRate: teamAResult.overs ? parseFloat(((safeNumber(teamAResult.runs) / parseOvers(teamAResult.overs)) || 0).toFixed(2)) : 0,
    };
    match.score.teamB = {
      battingTeam: 'teamB',
      runs: safeNumber(teamBResult.runs),
      wickets: safeNumber(teamBResult.wickets),
      balls: Math.floor(parseOvers(teamBResult.overs) * 6),
      extras: match.score?.teamB?.extras || 0,
      fours: match.score?.teamB?.fours || 0,
      sixes: match.score?.teamB?.sixes || 0,
      runRate: teamBResult.overs ? parseFloat(((safeNumber(teamBResult.runs) / parseOvers(teamBResult.overs)) || 0).toFixed(2)) : 0,
    };
    ...
    // Aggregate player career stats from this match
