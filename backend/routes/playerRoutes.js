...
router.get("/top-performers", async (req, res) => {
  try {
    ...
    // If seasonId was provided but no teams exist, return empty
    if (noTeamsFound) {
    ...
    // Default: return both runs and wickets
...
// Admin: aggregate career stats from completed matches
const { aggregateAllPlayerStats, aggregatePlayerStatsForMatch } = require('../utils/aggregatePlayerStats');

router.post('/aggregate-stats', async (req, res) => {
  try {
    const { matchId } = req.body;
    let result;
    if (matchId) {
      result = await aggregatePlayerStatsForMatch(matchId);
    } else {
      result = await aggregateAllPlayerStats();
    }
    res.json({ success: true, ...result });
  } catch (err) {
    console.error('Failed to aggregate player stats:', err);
